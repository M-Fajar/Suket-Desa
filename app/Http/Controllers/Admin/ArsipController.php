...
class ArsipController extends Controller
{
    ...
    public function kelahiranLihat($id)
    {
        $kelahiran = Kelahiran::find($id);
        $data['kelahiran'] = $kelahiran;
        $data['kades'] = Kades::where('status', 1)->first();

        $pdf = \PDF::loadView('arsip.kelahiran', $data);
        return $pdf->stream('Surat Kelahiran.pdf');
    }
    public function kelahiranDownload($id)
    {
        $kelahiran = Kelahiran::find($id);
        $data['kelahiran'] = $kelahiran;
        $data['kades'] = Kades::where('status', 1)->first();
        ...
        $pdf = \PDF::loadView('arsip.kelahiran', $data);
        return $pdf->download('Surat Kelahiran.pdf');
    }
    ...
    {
        $kelahirans = Kelahiran::where('acc', 1)->latest()->get();
        return view(
            'admin.arsip.non-harian.kelahiran',
            compact('kelahirans')
        );
    }
    ...
    public function kelahiranLaporan()
    {
        return Excel::download(new KelahiranExport, 'kelahiran-keluar.xlsx');
    }
}
