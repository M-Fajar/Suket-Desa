...
class AntrianController extends Controller
{
    ...
    public function updateKelahiran(Request $request, $id)
    {
        $kades_aktif = Kades::where('status', 1)->first();
        ...
        if (is_null($kades_aktif->ttdcap)) {
            ...
            // penomoran surat
            $noindex = '474.1';
            $stringtetap = 'Ds.2008-37';
            $bulan = (int)date('n');
            $bulanromawi = $this->getRomawi($bulan);
            $tahun = date('Y');

            // Nomor yang bertambah otomatis dan diulang dari 1 ketika awal bulan
            $urutan = Kelahiran::max('nomor');

            $skusblm = Kelahiran::where('nomor', $urutan)->first();
    ...
    public function kelahiranTolak($id)
    {
